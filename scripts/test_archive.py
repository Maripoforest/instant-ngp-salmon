...
def reload_scene(args, testbed, frame_n):
    testbed.clear_training_data()
    # testbed.reset()

    # Dynamic Salmon ===========================================================
    # frame_name = str(frame_n).zfill(4)
    # destination_folder = "./data/nerf/salmon/images"
    # num_images_to_transfer = 19        
    # source_folder = "./data/nerf/salmon/frames/" + frame_name       
    # transfer_images(source_folder, destination_folder, num_images_to_transfer)
    # Dynamic Salmon ===========================================================
...
def transfer_images(source_folder, destination_folder, num_images_to_transfer):
    if os.path.exists(destination_folder):
        shutil.rmtree(destination_folder)
    os.makedirs(destination_folder)
    source_path = source_folder
    if os.path.isdir(source_path):
        for i in range(num_images_to_transfer):
...
def main(n_steps, args, testbed, old_training_step, tqdm_last_update):
    if n_steps > 0:
        with tqdm(desc="Training", total=n_steps, unit="step") as t:
            ...
            while testbed.frame():
                if time.time() - _t > 7:
                    _t = time.time()
                    reload_scene(args, testbed=testbed, frame_n = frame_n)
                    frame_n += 1
                    frame_n %= 150
...
if __name__ == "__main__":
    ...
    # Dynamic Salmon ===========================================================
    ppl = NeRFPipeline()
    # main_thread = threading.Thread(target=main, args=(n_steps, args, testbed, old_training_step, tqdm_last_update))
    # main_thread.start()
