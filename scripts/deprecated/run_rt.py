...
def safe_copy_file(source_path, destination_path):
    """
    Copies a file from the source path to the destination path. If a file with the same name exists
    at the destination, it is overwritten. The function ensures the copy process is completed
    before proceeding.
    ...
    None
    """
    # Copy the file, ensuring that it's overwritten if it already exists
    shutil.copy2(source_path, destination_path)

    # Verify that the copy is complete
    if not os.path.exists(destination_path):
        raise Exception("Copy failed: Destination file does not exist.")
    if os.path.getsize(source_path) != os.path.getsize(destination_path):
        raise Exception("Copy failed: Source and destination file sizes do not match.")
        ...
        with tqdm(desc="Training", total=n_steps, unit="step") as t:
            while testbed.frame():
                ...
                if training_step % 50 == 0:              
                    safe_copy_file("./data/nerf/fox/transforms_less/transforms.json", "./data/nerf/fox/transforms.json")
                    print("less")
                if training_step % 70 == 0:
                    safe_copy_file("./data/nerf/fox/transforms_more/transforms.json", "./data/nerf/fox/transforms.json")
                    print("more")
