...
def reload(testbed, args, i):
	if args.scene:
		...
		testbed.shall_train_encoding = True
		testbed.shall_train_network = True

def copy_json_file(i):
    source_dir = 'data/nerf/socket2/output_frames'
    target_dir = 'data/nerf/socket2'
    os.makedirs(target_dir, exist_ok=True)
    source_file = os.path.join(source_dir, f'transforms_{i}.json')
    target_file = os.path.join(target_dir, 'transforms.json')
...
if __name__ == "__main__":
	...
	testbed.create_empty_nerf_dataset(1000, aabb_scale = 4)
	...
	if n_steps > 0:
		with tqdm(desc="Training", total=n_steps, unit="steps") as t:
			while testbed.frame():
				...
				if now - initial_time > 2 and testbed.shall_train:
					reload(testbed, args, count)
					count += 1
					initial_time = now 
